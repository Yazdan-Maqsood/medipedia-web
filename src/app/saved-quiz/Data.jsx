...
export default function Data({ user_id }) {
    ...
    const promptConfirmation = (redirect_url, test_id) => {
        MySwal.fire({
            title: "Saved Quiz",
            ...
            if (result.isConfirmed) {
                resetdone(redirect_url, test_id);
            } else {
                router.push(`/quiz/${redirect_url}`)
            }
        })
    }

    const resetdone = async (redirect_url, test_id) => {
        try {
            const formData = new FormData();
            formData.append("test_id", test_id);
            // ✅ Fix: Changed 'session.user.id' to 'user_id' prop directly
            formData.append("user_id", user_id);
            ...
            if (responseData.success) {
                ...
                router.push(`/quiz/${redirect_url}?refresh=true`);
            }
        } catch (error) {
                                            ...
                                            <tr key={index}>
                                                ...
                                                <td className="cart-price">
                                                    <span className="amount">
                                                        <i onClick={() => promptConfirmation(`${item.spec_slug}/${item.book_slug}/${item.paper_slug}/${item.test_slug}`, item.test_id)} className="fa fa-eye" style={{cursor: 'pointer'}}></i>
                                                    </span>
                                                </td>
                                            </tr>
                                        ))}
