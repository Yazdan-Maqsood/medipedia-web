  ...
  return (
    <>
      <section className="courses-category-area ptb-50">
        ...
        {params.param == 1  ? (
          <>
            <Apply title="To get all books code at once just pay" price={data.price} slug="mock-bundle-1" ></Apply>
            <br />
          </>
        ) : params.param == 2 ? (
          <>
            <Apply title="To get all books code at once just pay" price={data.price} slug="mock-bundle-2" ></Apply>
          </>
        ) : (
          <>
          </>
...
async function getData(params, user_id) {
  try {
    var mockBundle;
    if (params.param==1) {
        mockBundle="mock-bundle-1";
    }else if(params.param==2){
        mockBundle="mock-bundle-2";
    }
    console.log(mockBundle);
    
    const formData = new FormData();
    formData.append("slug", "mock-test-packages");
    formData.append("mockBundle", mockBundle);
