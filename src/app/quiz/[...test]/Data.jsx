...
export default function page(props) {
    ...
    const [total, settotal] = useState(1);          // total use mcqs from data
    ...
    const [actualduration, setactualDuration] = useState({ hours: 0, minutes: 0, seconds: 0 });
    const [shuuflephase, setshuuflephase] = useState(false);

    console.log(actualduration);
    console.log(duration);

    function calculateTimeDifference(actualDuration, duration) {
        // Convert both durations to seconds
        const actualTotalSeconds = actualDuration.hours * 3600 + actualDuration.minutes * 60 + actualDuration.seconds;
        const totalSeconds = duration.hours * 3600 + duration.minutes * 60 + duration.seconds;

        // Calculate the difference in seconds
        const differenceInSeconds = Math.abs(actualTotalSeconds - totalSeconds);
    ...
    const handleNext = () => {
        if (!shuuflephase) {
            ...
            if (skiparr[indexskip].setoptans === "") {
                toast.error("If you want to proceed next then skip this first!")
            } else {
                setrefresh(false);
    ...
    const resetdone = async () => {
        ...
        settotaldata(actualDataLength);
        setshuuflephase(false)
    ...
    const calculateTimeBasedOnLength = (length) => {
        ...
        setactualDuration({ hours, minutes: 0, seconds: 0 });
        setDuration({ hours, minutes: 0, seconds: 0 });
    };
